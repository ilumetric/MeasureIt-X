...
from math import fabs, degrees, radians, sqrt, cos, sin, pi, atan2
...
def draw_segments(context, myobj, op, region, rv3d):
    if op.measureit_num > 0:
        ...
        # Global appearance settings (apply to all segments)
        fsize = scene.measureit_font_size
        faln = scene.measureit_font_align
        glinew = scene.measureit_line_width
        gfoffset = scene.measureit_font_offset
        garrow_a = scene.measureit_glarrow_a
        garrow_b = scene.measureit_glarrow_b
        garrow_s = scene.measureit_glarrow_s
        # Lines are always drawn in 3D world space; X-Ray controls depth testing:
        #   X-Ray ON  → no depth test, lines render on top of everything.
        #   X-Ray OFF → depth test enabled, lines are occluded by scene geometry.
        depth_test = not scene.measureit_xray
        fang = 0.0
        ...
        for idx in range(op.measureit_num):
            ...
            if ms.glview is True and ms.glfree is False:
                ...
                try:
                    ...
                    # line setup
                    # ------------------------------------
                    imm_set_line_width(glinew)

                    # ------------------------------------
                    # Text rotation + perpendicular offset along the segment
                    # ------------------------------------
                    seg_rot = 0.0
                    perp_x, perp_y = 0.0, 1.0
                    if ms.gltype in {1, 12, 13, 14} \
                            and screen_point_v1 is not None and screen_point_v2 is not None:
                        sdx = screen_point_v2[0] - screen_point_v1[0]
                        sdy = screen_point_v2[1] - screen_point_v1[1]
                        seg_len = sqrt(sdx * sdx + sdy * sdy)
                        if seg_len > 0.0:
                            line_rot = atan2(sdy, sdx)
                            # keep text upright
                            if line_rot > pi / 2:
                                line_rot -= pi
                            elif line_rot < -pi / 2:
                                line_rot += pi
                            seg_rot = line_rot
                            # perpendicular to the line, biased to point up in screen
                            perp_x = -sdy / seg_len
                            perp_y = sdx / seg_len
                            if perp_y < 0.0:
                                perp_x = -perp_x
                    ...
                    if ms.gltype != 2 and ms.gltype != 9 and ms.gltype != 10 and ms.gltype != 11 and ms.gltype != 20:
                        ...
                        try:
                            ...
                            if tmp_point is None:
                                pass
                            txtpoint2d = tmp_point[0] + perp_x * gfoffset, tmp_point[1] + perp_y * gfoffset
                            # Scale
                            if scene.measureit_scale is True:
                            ...
                            if scene.measureit_gl_show_d is True or scene.measureit_gl_show_n is True:
                                draw_text(myobj, txtpoint2d, msg, rgba, fsize, faln, seg_rot)
                            ...
                            if locflag is True and ms.glocwarning is True:
                                ...
                                if ms.glocz is True:
                                    txt += "Z"
                                txt += "]"
                                draw_text(myobj, txtpoint2d, txt, rgba, fsize - 1, text_rot=seg_rot)
                    ...
                    if ms.gltype == 1:  # Segment
                        draw_line(a_p1, v11, rgba, world_3d=True, rv3d=rv3d, depth_test=depth_test)
                        draw_line(b_p1, v22, rgba, world_3d=True, rv3d=rv3d, depth_test=depth_test)
                        draw_arrow(screen_point_v1, screen_point_v2, rgba, a_size, a_type, b_type,
                                   world_v1=v1, world_v2=v2, rv3d=rv3d, region=region, depth_test=depth_test)
                    ...
                    if ms.gltype == 2:  # Label
                        draw_line(v11a, v11b, rgba, world_3d=True, rv3d=rv3d, depth_test=depth_test)
                        draw_arrow(screen_point_ap1, screen_point_v11, rgba, a_size, a_type, b_type,
                                   world_v1=a_p1, world_v2=v11, rv3d=rv3d, region=region, depth_test=depth_test)
                    ...
                    if ms.gltype == 3 or ms.gltype == 4 or ms.gltype == 5 or ms.gltype == 8 \
                        ...
                        draw_arrow(screen_point_ap1, screen_point_bp1, rgba, a_size, a_type, b_type,
                                   world_v1=a_p1, world_v2=b_p1, rv3d=rv3d, region=region, depth_test=depth_test)
                    ...
                    if ms.gltype == 9:  # Angle
                        ...
                        draw_line(mp1, an_p2, rgba, world_3d=True, rv3d=rv3d, depth_test=depth_test)
                        draw_line(an_p2, mp2, rgba, world_3d=True, rv3d=rv3d, depth_test=depth_test)
                        draw_line(mp1, mp2, rgba, world_3d=True, rv3d=rv3d, depth_test=depth_test)
                    ...
                    if ms.gltype == 11:  # arc
                        ...
                        if ms.glarc_rad is True:
                            if ms.glarc_extrad is False:
                                ...
                                draw_arrow(screen_point_ap1, screen_point_pe, rgba, a_size, a_type, b_type,
                                           world_v1=a_p1, world_v2=pe, rv3d=rv3d, region=region, depth_test=depth_test)
                        ...
                        for i in range(int(n_step)):
                            ...
                            if i == 0:
                                draw_arrow(screen_point_p1, screen_point_p2, rgba, ms.glarc_s, ms.glarc_a, "99",
                                           world_v1=p1_, world_v2=p2_, rv3d=rv3d, region=region, depth_test=depth_test)
                            elif i == int(n_step) - 1:
                                draw_arrow(screen_point_p1, screen_point_p2, rgba, ms.glarc_s, "99", ms.glarc_b,
                                           world_v1=p1_, world_v2=p2_, rv3d=rv3d, region=region, depth_test=depth_test)
                            else:
                                draw_line(p1_, p2_, rgba, world_3d=True, rv3d=rv3d, depth_test=depth_test)
                        ...
                        # Draw close lines
                        if ms.glarc_full is False:
                            draw_line(p_01a, p_01b, rgba, world_3d=True, rv3d=rv3d, depth_test=depth_test)
                            draw_line(p_02a, p_02b, rgba, world_3d=True, rv3d=rv3d, depth_test=depth_test)
...
#
# ------------------------------------------
def get_area_and_paint(myvertices, myobj, obverts, region, rv3d, rgba, ms_scale, depth_test=False):
    mymesh = myobj.data
    totarea = 0
    if len(myvertices) > 3:
        ...
        for t in tris:
            ...
            draw_triangle(p1, p2, p3, rgba, world_3d=True, rv3d=rv3d, depth_test=depth_test)
...
def draw_text(myobj, pos2d, display_text, rgba, fsize, align='L', text_rot=0.0):
    ...
    # Shadow without offset to separate text from background
    ...
    for line in mylines:
        ...
        # rotate the alignment offset around the anchor so anchoring works at any angle
        draw_x = x_pos + x_offset * cos_r
        draw_y = new_y + x_offset * sin_r
        # Draw
...
#
# -------------------------------------------------------------
def draw_line(v1, v2, rgba, world_3d=False, rv3d=None, depth_test=False):
    ...
    try:
        ...
        if world_3d and rv3d is not None:
...
#
# -------------------------------------------------------------
def draw_triangle(v1, v2, v3, rgba, world_3d=False, rv3d=None, depth_test=False):
...
def draw_arrow(v1, v2, rgba, size=20, a_typ="1", b_typ="1",
               world_v1=None, world_v2=None, rv3d=None, region=None, depth_test=False):
    if v1 is None or v2 is None:
        return
    ...
    v1a = (v1i[0] * cos(rad_a) - v1i[1] * sin(rad_a) + v1[0],
           v1i[1] * cos(rad_a) + v1i[0] * sin(rad_a) + v1[1])
    v1b = (v1i[0] * cos(rad_b) - v1i[1] * sin(rad_b) + v1[0],
           v1i[1] * cos(rad_b) + v1i[0] * sin(rad_b) + v1[1])
    ...
    v2a = (v2i[0] * cos(rad_a) - v2i[1] * sin(rad_a) + v2[0],
           v2i[1] * cos(rad_a) + v2i[0] * sin(rad_a) + v2[1])
    v2b = (v2i[0] * cos(rad_b) - v2i[1] * sin(rad_b) + v2[0],
           v2i[1] * cos(rad_b) + v2i[0] * sin(rad_b) + v2[1])

    use_3d = (world_v1 is not None and world_v2 is not None
              and rv3d is not None and region is not None)

    if use_3d:
        # Deproject arrowhead screen points onto the plane parallel to the view
        # passing through each anchor — keeps size in pixels but moves the geometry
        # into world space so it depth-tests with the scene.
        w_v1a = view3d_utils.region_2d_to_location_3d(region, rv3d, v1a, world_v1)
        w_v1b = view3d_utils.region_2d_to_location_3d(region, rv3d, v1b, world_v1)
        w_v2a = view3d_utils.region_2d_to_location_3d(region, rv3d, v2a, world_v2)
        w_v2b = view3d_utils.region_2d_to_location_3d(region, rv3d, v2b, world_v2)

        if a_typ in ("1", "2", "3"):
            draw_line(world_v1, w_v1a, rgba, world_3d=True, rv3d=rv3d, depth_test=depth_test)
            draw_line(world_v1, w_v1b, rgba, world_3d=True, rv3d=rv3d, depth_test=depth_test)
        if b_typ in ("1", "2", "3"):
            draw_line(world_v2, w_v2a, rgba, world_3d=True, rv3d=rv3d, depth_test=depth_test)
            draw_line(world_v2, w_v2b, rgba, world_3d=True, rv3d=rv3d, depth_test=depth_test)

        draw_line(world_v1, world_v2, rgba,
                  world_3d=True, rv3d=rv3d, depth_test=depth_test)
    else:
        # Screen-space fallback (used by debug overlays etc.)
        if a_typ in ("1", "2", "3"):
